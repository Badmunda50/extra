...
async def check_blacklisted_words_on_edit(_, m: Message):
    ...
    LOGGERR.info(f"Edited message detected from user {m.from_user.id} in chat {m.chat.id}")

    if not blacklisted_words:
        LOGGERR.info("No blacklisted words found")
        return

    for word in blacklisted_words:
        if word in m.text.lower():
            LOGGERR.info(f"Blacklisted word '{word}' found in edited message")
            if action == "none":
                LOGGERR.info("Action is set to 'none', not deleting the message")
                return

            await m.delete()
            LOGGERR.info("Message deleted")

            if action == "ban":
                await app.kick_chat_member(m.chat.id, m.from_user.id)
                LOGGERR.info("User banned")
            elif action == "kick":
                await app.kick_chat_member(m.chat.id, m.from_user.id)
                await app.unban_chat_member(m.chat.id, m.from_user.id)
                LOGGERR.info("User kicked")
            elif action == "mute":
                await app.restrict_chat_member(m.chat.id, m.from_user.id, permissions=False)
                LOGGERR.info("User muted")
            elif action == "warn":
                reason = db.get_reason()
                await m.reply_text(f"Warning! You used a blacklisted word. Reason: {reason}")
                LOGGERR.info("User warned")
